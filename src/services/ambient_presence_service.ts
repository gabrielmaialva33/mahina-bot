...
export class AmbientPresenceService {
  ...
  async sendMemberWelcome(member: GuildMember): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `chegou sangue novo no **${member.guild.name}**.\n` +
          'seja útil, seja engraçado ou pelo menos não seja npc.'
      )
      .setFooter({
        text: 'a Mahina já tá de olho no teu estilo',
  ...
  async sendChannelSpark(channel: TextChannel): Promise<void> {
    ...
    const message = `canal novo cheirando a tinta.\nveremos quanto tempo leva pra descambar kkkkk`
